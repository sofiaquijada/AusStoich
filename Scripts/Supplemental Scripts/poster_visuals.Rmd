...
env <- c("SN_total_0_30", "SP_total_0_30", "SOC_total_0_30",
  "CEC_total_0_30", "AP_total_0_30",
  "NPP", "MAT", "PPT", "AET",
  "precipitation_seasonality", "temp_seasonality")

for (indep in env) {
  p <- ggplot(pruned, aes(x = !!sym(indep), y = ln_leaf_N, color = species_binom)) +
...
pruned <- prune_genera(aus_data, 96)
...
for (indep in env){
  p <- ggplot(pruned, aes(x = !!sym(indep), y = ln_leaf_N, color = genus)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE)
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
pruned <- prune_family(aus_data, 108)
...
for (indep in env){
  p <- ggplot(pruned, aes(x = !!sym(indep), y = ln_leaf_N, color = family)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE)
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
for (indep in env) {
  p <- ggplot(aus_data, aes(x = !!sym(indep), y = (leaf_N_per_dry_mass))) +
    geom_point(alpha = 0.6) +
    geom_smooth(method = "lm", se = FALSE) +
theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Env-trait linregs on top taxonomic levels:
...
ggplot(aus_data, aes(x = SN_total_0_30, y = (leaf_N_per_dry_mass))) +
    geom_point(alpha = 0.6) +
    geom_smooth(method = "lm", se = FALSE) +
theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
